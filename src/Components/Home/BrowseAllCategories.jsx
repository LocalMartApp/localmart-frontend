...
const BrowseAllCategories = ({ isCategoryOpen , closeCategory }) => {
    ...
    const filteredCategories = allCategories.filter(item =>
        item.rightText.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  return (
    ...
    <div className="outer-main-section-browse-all-cates">
        <div className={`main-browse-all-categories-section fixed top-0 w-[70%] h-full bg-LightBlue duration-500 z-[999999] p-40p ${isCategoryOpen ? 'right-0 visible' : '-right-full invisible'}`}>
                            ...
                            <div className="single-category-item bg-white rounded-10p p-[15px]" key={index}>
                                <div className="inner-single-category-item flex items-center gap-5">
                                    ...
                                    <div className="right-side-category-name">
                                        <p className='text-Black font-medium'>{items.rightText}</p>
                                    </div>
                                </div>
                            </div>
                        )
